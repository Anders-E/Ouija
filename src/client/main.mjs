...
function initScene(scene, renderer) {
    ...
    //light
    var ambientLight = new THREE.AmbientLight( 0x8aa1ff, 0.25); // soft white light
    scene.add( ambientLight );

    var light = new THREE.PointLight(0x66b2ff, 1);
    light.position.set(6.12, 8.76, 15.39);
    scene.add(light);

    window.lightningLight = new THREE.PointLight(0x66b2ff, 0);
    lightningLight.position.set(6.12435, 8.759488, 15.3907);
    ...
    loader.load(
        ...
        function(gltf) {
            ...
            window.cameraAction = mixer.clipAction(THREE.AnimationClip.findByName(window.clips, 'Action.002'));
            cameraAction.timeScale = 2; // add this
